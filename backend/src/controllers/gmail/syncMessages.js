 ...
 */
export const syncMessagesController =
  (googleOAuth, prisma, linkDetectionService) => async (req, res) => {
        ...
        for (const message of gmailMessages) {
          try {
              ...
              for (const attachment of message.attachments) {
                if (attachment.fileId) {
                  ...
                  if (file) {
                    ...
                    console.log(
                      `File not found in database: ${attachment.fileId} (${attachment.type})`
                    );
                  }
                }
              }
            ...
            try {
              ...
              const linkResults = await linkDetectionService.processEmailText(
                savedMessage.id,
                emailText,
                userId
              );
              ...
              if (linkResults.errors.length > 0) {
                console.warn(
                  `Link detection errors for email ${savedMessage.id}:`,
                  linkResults.errors
                );
              }
            } catch (linkError) {
              console.warn(
                `Failed to detect links for email ${savedMessage.id}:`,
                linkError.message
              );
            }
