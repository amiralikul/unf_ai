    ...
    const syncResults = await prisma.$transaction(async tx => {
      ...
      for (const board of trelloBoards) {
        try {
          ...
          const cards = await trelloService.getCardsForBoard(
            trello_api_key,
            trello_token,
            board.id
          );
          ...
          for (const card of cards) {
            try {
              ...
              try {
                ...
                const linkResults = await linkDetectionService.processCardText(
                  savedCard.id,
                  cardText,
                  userId
                );
                ...
                if (linkResults.errors.length > 0) {
                  console.warn(
                    `Link detection errors for card ${savedCard.id}:`,
                    linkResults.errors
                  );
                }
              } catch (linkError) {
